(() => {
  ...
  let sizeMap, drawMap, pickSystemAt;
  function initChartRenderer() {
    const renderer = globalThis.SkiffChartRenderer.setup({
    ...
    sizeMap = renderer.sizeMap;
    drawMap = renderer.drawMap;
    pickSystemAt = renderer.pickSystemAt;
  }


  let renderShipPanel, renderAgentActionLog, renderQuests, renderSkillsBox;
  function initTabsRenderer() {
    const renderer = globalThis.SkiffTabsRenderer.setup({
      el, sys, state, ui, themeColors, fuelReachDistance, SYSTEMS: globalThis.SkiffSystems, 
      canJumpTo, isVisited, riskFill, canSeeTrade, bestLaneEdge, 
      peekPrices, WP, CF, fuelCost, dist, inRange, SECTOR_RADIUS,
      hull, GOODS: globalThis.SkiffGoods, SHIPS: globalThis.SkiffShips, netWorth, cargoUsed, qtyFor, doSellAll, doFillCheap, doSellExpensive, doRefuel, doRepair, doRearm,
      doYardTake, godEnabled, currentTheme: () => ui.theme, applyTheme, applyPilot, currentPilot, tickSkill, CR: globalThis.SkiffCrew, SK: globalThis.SkiffSkills, formatTickerLine: globalThis.SkiffAgentActionLog ? globalThis.SkiffAgentActionLog.formatTickerLine : null,
      sellQty, buyQty, doMarketBuy, doMarketSell,
      VERSION
    });
    renderShipPanel = renderer.renderShipPanel;
    renderAgentActionLog = renderer.renderAgentActionLog;
    renderQuests = renderer.renderQuests;
    renderSkillsBox = renderer.renderSkillsBox;
  }
